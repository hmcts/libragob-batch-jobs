...
echo "Script Version 27.2: premise endpoint check"
...
echo "pod_running=$pod_running"
if [[ $pod_running == 1 ]];then
  if [[ $op_env == test ]];then
    kubectl -n met -it exec $pod_hash -- wget -O- "https://libra-onpremise-gob-gateway.staging.internal.hmcts.net/themisgateway/service/themissoapgatewayapi?wsdl" > ${OPDIR}onpremise_endpoint_check
  else
...
active_threshold=35 # 28 seen when 372K batch running on 25/07/2025 so tuned from 25 to 35. 23 seen at 12:40 19/12/2024 when two big bundled updates on 105 & 112 were playing in so tuned from 18 to 25.
null_threshold=15
...
elif [[ $schema_id == 57 ]] || [[ $schema_id == 103 ]] || [[ $schema_id == 26 ]] || [[ $schema_id == 119 ]] || [[ $schema_id == 28 ]] || [[ $schema_id == 60 ]] || [[ $schema_id == 139 ]] || [[ $schema_id == 24 ]] || [[ $schema_id == 22 ]] || [[ $schema_id == 29 ]];then
