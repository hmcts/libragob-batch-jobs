...
for loopc in 1 2 3 4;do
  ...
  if [[ `echo $confiscation_rec | grep -v "missing" | grep -v "ERRORs" | wc -l` == 1 ]] && [[ `echo $fines_rec | grep -v "missing" | grep -v "ERRORs" | wc -l` == 1 ]] && [[ `echo $maintenance_rec | grep -v "missing" | grep -v "ERRORs" | wc -l`== 1 ]]
