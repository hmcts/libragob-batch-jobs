...
echo "Script Version 25.7 dupe seq num new sql #2"
...
#  psql "sslmode=require host=${event_host} dbname=${event_db} port=${event_port} user=${event_username} password=${event_password}" --file=/sql/12AZUREDB_AMD_ora_rowscn_bug_seq_nums_fix.sql
   psql "sslmode=require host=${event_host} dbname=${event_db} port=${event_port} user=${event_username} password=${event_password}" -c "call fix_duplicate_seq_nos\(\)" -e > /tmp/ams-reporting/12AZUREDB_AMD_ora_rowscn_bug_seq_nums.csv
